    ...
    const promise = Promise.try(() => {
      ...
      .then(res => {
        if (res === "regenOnlyFail") {
          logger.warn(
            chalk.yellow(`optional check ${pkgId} regen only optional false - auto failed`)
          );
          return { passed: false };
        }
        ...
        if (checked) {
          ...
          return ls
            ...
            .catch(err => {
              logger.warn(chalk.yellow(`optional check ${pkgId} preinstall script failed`));
              return { passed: false, err };
            });
        } else {
          // no preinstall script, always pass
          logger.verbose(
            chalk.green(`optional check ${pkgId} no preinstall script - automatic pass`)
          );
          return { passed: true };
        }
      })
