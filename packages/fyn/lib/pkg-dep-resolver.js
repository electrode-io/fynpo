...
class PkgDepResolver {
  ...
  makePkgDepItems(pkg, parent, dev, noPrefetch) {
    ...
    const makeDepItems = (deps, dsrc) => {
      ...
      for (const name in deps) {
        if (!bundled || bundled.indexOf(name) < 0) {
          const opt = { name, semver: deps[name], src, dsrc };
    ...
    //
    const filterOptional = deps => {
      if (!pkg.optionalDependencies) return deps;
      deps = Object.assign({}, deps);
      _.each(pkg.optionalDependencies, (v, n) => {
        delete deps[n];
      });
      return deps;
    };
    ...
    return {
      name: pkg.name,
      dep: pkg.dependencies && makeDepItems(filterOptional(pkg.dependencies), "dep"),
      dev: dev && pkg.devDependencies && makeDepItems(pkg.devDependencies, "dev"),
      opt: pkg.optionalDependencies && makeDepItems(pkg.optionalDependencies, "opt")
    };
  }
