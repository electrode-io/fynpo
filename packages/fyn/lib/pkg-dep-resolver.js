...
class PkgDepResolver {
  constructor(pkg, options) {
    ...
    //
    // TODO: But we can start fetching meta for each package as soon as they are
    // determined to be within the dep tree.
    //
    ...
    if (depth > 1) {
      ...
      Object.keys(parentDepth).forEach(x => {
        if (parentDepth[x].deps) {
          parentDepth[x].deps.forEach(x2 => this.addDepOfDep(x2[0], x2[1]));
          parentDepth[x].deps = undefined;
        }
      });
    }
  ...
  addResolving(pkg, section, dsrc, parent) {
    ...
    for (const name in dep) {
      if (!bundled || bundled.indexOf(name) < 0) {
        const opt = { name, semver: dep[name], src, dsrc };
        const newItem = new DepItem(opt, parent);
        // logger.info("new item depth", name, newItem.depth);
        const depthData = this._depthResolving[newItem.depth];
        if (!depthData) {
          this._depthResolving[newItem.depth] = {
            [name]: { items: [newItem] }
          };
        } else if (!depthData[name]) {
          depthData[name] = { items: [newItem] };
        } else {
          depthData[name].items.push(newItem);
        }
  ...
  addPkgDep(mPkg, dev, parent) {
    ...
    this.addResolving(mPkg, "dependencies", "dep", parent);
    if (dev) {
      this.addResolving(mPkg, "devDependencies", "dev", parent);
    }
    if (this.addResolving(mPkg, "optionalDependencies", "opt", parent) > 0) {
      this._promiseQ.addItem(PromiseQueue.pauseItem, true);
    }
  }

  addDepOfDep(mPkg, parent) {
    this.addPkgDep(mPkg, false, parent);
