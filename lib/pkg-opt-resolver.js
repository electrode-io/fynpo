...
class PkgOptResolver {
  ...
  optCheck(data) {
    ...
    this._inflights[pkgId] = checkPkg(installedPath)
      ...
      .catch(() => {
        ...
        return this._fyn.pkgSrcMgr
          ...
          .then(res => {
            ...
            const tarXOpt = { file: res.fullTgzFile, strip: 1, C: installedPath };
            return Promise.try(() => Tar.x(tarXOpt))
              .then(() => checkPkg(installedPath))
              .tap(x => assert(x, "opt-resolver: newly installed package version didn't match!?"));
          });
      })
      .then(res => {
      ...
      .tap(res => {
        if (this._checkedPkgs[pkgId] === undefined) {
