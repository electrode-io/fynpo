...
class PkgDepLinker {
  ...
  async addSubNodeModules(depInfo, fvDeps) {
    ...
    const subjectNmDir = await this._fyn.createSubNodeModulesDir(
      this._fyn.getInstalledPkgDir(depInfo.name, depInfo.version)
    );
    ...
    for (const di of fvDeps) {
      const diDir = this._fyn.getInstalledPkgDir(di.name, di.version);
  ...
  async loadPkgDepData(depInfo) {
    ...
    const installedDir = this._fyn.getInstalledPkgDir(depInfo.name, depInfo.version);
