...
class PkgSrcManager {
    ...
    const pacoteRequest = () => {
      return pacote
        .packument(
          ...
          this.getPacoteOpts({
            "full-metadata": true,
            // if we got here, we've done our own cache check, so
            // set prefer online flag, to make pacote go out to network
            // to see if there's an updated pakument in the registry
            "prefer-online": true,
            "fetch-retries": 3
          })
        )
  ...
  fetchMeta(item) {
    ...
    const cacheKey = `make-fetch-happen:request-cache:https://registry.npmjs.org/${escapedName}`;
  ...
  async getCentralPackage(integrity, pkgInfo) {
    ...
    const tarStream = () => {
      return integrity
        ...
        : this.pacoteTarballStream(tarId, integrity);
    };
