  ...
  netRetrieveMeta(qItem) {
    ...
    const doRequest = () => {
      ...
      const promise = Promise.try(() =>
        ...
        .catch(err => {
          retries++;
          const display = `Network error fetching meta of ${pkgName}`;

          if (retries < 5) {
            logger.addItem({ name: networkRequestId, display, color: "red" });
            // try again
            logger.updateItem(networkRequestId, err.message + `; Retrying ${retries}`);
            return doRequest().tap(() => logger.removeItem(networkRequestId));
          } else {
