...
class PkgDistFetcher {
  ...
  async findPkgInNodeModules(pkg) {
    ...
    const result = { foundAtTop: false, search: [] };
    // first check if node_modules contains a copy already
    // - first promoted true, then false.
  ...
  //
  // Handles putting pkg into node_modules
  // - This handles extracting pkg during resolving phase when a pkg's promoted is unknown, but
  //   it could've been exist at top level already from previous install.
  // - So first thing is explicit check for top location.
  // - Next is explicit check at __fv_ location.
  // - Then finally fetch and extract tarball
  //
  async putPkgInNodeModules(pkg, check) {
    ...
    return await new Promise((resolve, reject) => {
      ...
      this.addSinglePkg({ pkg, listener, foundAtTop: find.foundAtTop });
    });
  }
}
