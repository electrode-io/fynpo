...
class PkgDepResolver {
  ...
  resolvePackage(item, meta, topKnownOnly, noLocal) {
      ...
      if (rversions.length > 0) {
        if (topKnownOnly) {
          resolved = _.find(rversions, v => {
            return kpkg[v] && kpkg[v].top ? satisfies(v, item.semver) : false;
          });
        } else {
  ...
  _resolveWithMeta(item, meta, force, noLocal) {
    const resolved = this.resolvePackage(item, meta, true, noLocal);
