...
class PkgDepResolver {
  ...
  semverSatisfies(v, semver) {
    const x = v.indexOf("-fynlocal");
    return Semver.satisfies(x > 0 ? v.substr(0, x) : v, semver);
  }
  ...
  resolvePackage(item, meta, topKnownOnly) {
      ...
      if (rversions.length > 0) {
        if (topKnownOnly) {
          resolved = _.find(rversions, v => {
            return kpkg[v] && kpkg[v].top ? this.semverSatisfies(v, item.semver) : false;
          });
        } else {
          resolved = _.find(rversions, v => this.semverSatisfies(v, item.semver));
        }
      }
    ...
    const searchMeta = () => {
      ...
      const find = versions =>
        versions && _.find(versions, v => this.semverSatisfies(v, item.semver));
